...
import { getOrnek, DISCLAIMER } from "@/data/ornekler";
      ...
      <div className="mx-auto max-w-5xl px-5 pb-8">
        <div className="overflow-hidden rounded-2xl border border-line bg-bg-raised/20">
          ...
          >
            ...
            <h2 className="display text-2xl font-bold text-ink md:text-3xl">{o.name}</h2>
            <p className="mt-2 text-muted">{o.tagline}</p>
            <p className="mt-1 text-sm text-faint">{o.sector} · {o.city}</p>
            <div className="mt-5 flex flex-wrap justify-center gap-3">
              <span
                className="inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold"
                style={{ background: o.accent, color: "#04121a" }}
              >
                <Phone className="h-4 w-4" /> Ara: {o.phone}
              </span>
          ...
          <div className="border-t border-line px-6 py-8">
            <h3 className="display mb-4 text-center text-lg font-bold text-ink">Hizmetlerimiz</h3>
          ...
          <div className="grid gap-px border-t border-line bg-line sm:grid-cols-3">
            ...
            <div className="flex items-start gap-2 bg-bg-raised/20 p-4 text-xs text-muted">
              ...
              <span>{o.city} · Harita bağlantısı</span>
            </div>
            <div className="flex items-start gap-2 bg-bg-raised/20 p-4 text-xs text-muted">
      ...
      <AnimatedSection className="band-petrol border-t border-line">
        <div className="mx-auto max-w-3xl px-5 py-14 text-center">
          <h2 className="display text-2xl font-bold text-ink md:text-3xl">
            Seninki de böyle olsun
          </h2>
          <p className="mx-auto mt-3 max-w-lg text-muted">
            Paketleri ve fiyatları gör, ya da WhatsApp'tan kısa bir mesaj at.
          </p>
          ...
          >
            <ArrowLeft className="h-4 w-4" /> Tüm örneklere dön
          </Link>
        </div>
      </AnimatedSection>
    </main>
